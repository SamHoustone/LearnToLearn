...
public class CustomisersVerbs : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCEVERBS", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("VERBSQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("VERBSA" + i, A[i].inputText.text);
        }
    }
}
