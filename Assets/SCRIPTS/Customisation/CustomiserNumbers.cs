...
public class CustomiserNumbers : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCENUMBERS", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("NUMBERSQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("NUMBERSA" + i, A[i].inputText.text);
        }
    }
}
