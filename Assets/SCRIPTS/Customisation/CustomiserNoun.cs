...
public class CustomiserNoun : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCENOUN", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("NOUNQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("NOUNA" + i, A[i].inputText.text);
        }
    }
}
