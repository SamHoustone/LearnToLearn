...
public class CustomiserConnrectors : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCECONNECTORS", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("CONNECTORSQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("CONNECTORSA" + i, A[i].inputText.text);
        }
    }
}
