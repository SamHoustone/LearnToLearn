...
public class CustomiserPrefix : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCEPREFIX", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("PREFIXQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("PREFIXA" + i, A[i].inputText.text);
        }
    }
}
