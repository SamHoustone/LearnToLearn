...
public class CustomiserGremetaria : MonoBehaviour
{
    private bool on;
    ...
    public void Go()
    {
        ...
        PlayerPrefs.SetInt("REFERENCEGEMATRIYA", number);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < number; i++)
        {
            PlayerPrefs.SetString("GEMATRIYAQ" + i, Q[i].inputText.text);
            PlayerPrefs.SetString("GEMATRIYAA" + i, A[i].inputText.text);
        }
    }
}
